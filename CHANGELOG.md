...
### Added
- Initial implementation of BananaMCP — a single `banana` binary with two subcommands:
  - `banana mcp` — starts the MCP stdio server (for use with AI coding assistants)
  - `banana test <prompt>` — generates an image and saves to `output.<ext>`
...
- `internal/generator` package with shared image generation logic used by both subcommands
- `cmd/banana/version.go` with semantic version `0.1.0`
- `Makefile` with `build`, `test`, `lint`, `fmt`, `tidy`, `clean`, `run-mcp`, `version`, `version-increment`, `release`, `push`, and `help` targets
- Install via `go install github.com/nealhardesty/bananamcp/cmd/banana@latest`
